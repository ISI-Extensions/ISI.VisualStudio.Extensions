...
{
	[Command(PackageIds.RecipeExtensions_ProjectPartialClass_AddDataTransferObjectRequestResponse_MenuItemId)]
	public class RecipeExtensions_ProjectPartialClass_AddDataTransferObjectRequestResponse_Command : BaseCommand<RecipeExtensions_ProjectPartialClass_AddDataTransferObjectRequestResponse_Command>
	{
				...
				if (inputDialogResult.GetValueOrDefault() && !string.IsNullOrWhiteSpace(inputDialog.Value))
				{
					...
					if (!string.IsNullOrWhiteSpace(classNamePrefix))
					{
						...
						if (!System.IO.Directory.Exists(partialClassDirectory))
						{
							...
							var recipes = new []
							{
								new Extensions_Helper.RecipeItem(System.IO.Path.Combine(directory, string.Format("{0}Request.cs", classNamePrefix)), RecipeExtensionsHelper.GetContent((isAsync ? nameof(RecipeExtensionsOptions.Project_AsyncDataTransferObjectRequest_Template) : nameof(RecipeExtensionsOptions.Project_DataTransferObjectRequest_Template)), projectDirectory, solutionRecipesDirectory, solutionDirectory), true),
								new Extensions_Helper.RecipeItem(System.IO.Path.Combine(directory, string.Format("{0}Response.cs", classNamePrefix)), RecipeExtensionsHelper.GetContent(nameof(RecipeExtensionsOptions.Project_DataTransferObjectResponse_Template), projectDirectory, solutionRecipesDirectory, solutionDirectory), true),
							};
