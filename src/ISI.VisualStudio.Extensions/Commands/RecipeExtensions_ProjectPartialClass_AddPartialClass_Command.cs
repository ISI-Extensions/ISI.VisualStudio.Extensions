					...
					if (!string.IsNullOrWhiteSpace(partialClassName))
					{
						...
						if (!System.IO.Directory.Exists(partialClassDirectory))
						{
							...
							if (partialClassName.EndsWith("Api", StringComparison.InvariantCulture))
							{
								...
								try
								{
									usings.Add(string.Format("DTOs = {0}.DataTransferObjects.{1}Repository", @namespace.TrimEnd(".Repository"), partialClassName.TrimEnd("Api")));
								}
								catch
								{
								}
							...
							var recipes = new []
							{
								new RecipeExtensions_Helper.RecipeItem(System.IO.Path.Combine(partialClassDirectory, string.Format("__{0}.cs", partialClassName)), RecipeExtensionsHelper.GetContent(nameof(RecipeExtensionsOptions.Project_AddPartialClass), projectDirectory, solutionRecipesDirectory, solutionDirectory), true),
							};
