				...
				if (inputDialogResult.GetValueOrDefault() && !string.IsNullOrWhiteSpace(inputDialog.NewPartialClassMethodName))
				{
					...
					if (!string.IsNullOrWhiteSpace(methodName))
					{
						...
						if (System.IO.Directory.Exists(partialClassDirectory))
						{
							...
							var recipes = new[]
							{
								new Extensions_Helper.RecipeItem(System.IO.Path.Combine(partialClassDirectory, string.Format("{0}{1}.cs", methodName, (isAsync ? "Async" : string.Empty))), GetContent((isAsync ? nameof(RecipeExtensionsOptions.ProjectPartialClass_AsyncPartialClassMethod_Template) : nameof(RecipeExtensionsOptions.ProjectPartialClass_PartialClassMethod_Template)), projectDirectory, solutionRecipesDirectory, solutionDirectory), true),
							};
