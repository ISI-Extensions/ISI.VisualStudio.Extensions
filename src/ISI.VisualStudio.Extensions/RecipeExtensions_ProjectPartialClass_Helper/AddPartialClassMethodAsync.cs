				...
				if (inputDialogResult.GetValueOrDefault() && !string.IsNullOrWhiteSpace(inputDialog.NewPartialClassMethodName))
				{
					...
					if (!string.IsNullOrWhiteSpace(methodName))
					{
						...
						if (System.IO.Directory.Exists(partialClassDirectory))
						{
							...
							var recipes = new[]
							{
								new Extensions_Helper.RecipeItem(System.IO.Path.Combine(partialClassDirectory, string.Format("{0}{1}.cs", methodName, (isAsync ? "Async" : string.Empty))), GetContent((isAsync ? nameof(Options.ProjectPartialClass_AsyncPartialClassMethod_Template) : nameof(Options.ProjectPartialClass_PartialClassMethod_Template)), projectDirectory, solutionRecipesDirectory, solutionDirectory), true),
							};
